...
from lib.modules import test as test, sample as sample, gen_sample as gen_sample
...
def parse_args():
    ...
    parser.add_argument('--pretrained_model_path', type=str, default= '../saved_models/data/model_save_file/state_epoch_1330.pth',
...
def main(args): 
    ...
    save_dir = osp.join(ROOT_PATH, 'imgs/{0}'.format(osp.join(str(args.CONFIG_NAME), 'test', stamp)))
    ...
    # sample(valid_dl, netG, text_encoder, save_dir, args.device, multi_gpus, args.z_dim, stamp)
    # if (args.multi_gpus==True) and (get_rank() != 0):
    #     None
    # else:
    #     print('FID: %.2f, CLIP_Score: %.2f' % (FID, TI_score*100))
    gen_sample(netG, text_encoder, save_dir, args.device, multi_gpus, args.z_dim)
