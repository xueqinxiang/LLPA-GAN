...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='GALIP',
                        help='the model for training')
    parser.add_argument('--state_epoch', type=int, default=1, #default=100,
                        help='state epoch')
    parser.add_argument('--batch_size', type=int, default=32,  #default=1024,
                        help='batch size')
    ...
    parser.add_argument('--clip_weight', type=float, default=0.2,
                        help='clip_weight in generator loss')
    parser.add_argument('--loss_gen', type=float, default=0.0,
                        help='loss_gen in generator loss')
...
def main(args):
    ...
    CLIP4trn, CLIP4evl, image_encoder, text_encoder, netG, netD, netC, GALIPNetD = prepare_models(args)
    print('**************G_paras: ',params_count(netG))
    print('**************D_paras: ',params_count(netD)+params_count(netC))
    #print('**************D_paras: ', params_count(GALIPNetD) + params_count(netC)) # ******GALIP OK
    ...
    #D_params = list(GALIPNetD.parameters()) + list(netC.parameters()) # ******GALIP OK
    D_params = list(GALIPNetD.parameters()) + list(netC.parameters()) + list(netD.parameters())
    ...
    if args.state_epoch!=1:
        ...
        #netG, netD, netC, GALIPNetD, optimizerG, optimizerD = load_models_opt(netG, netD, netC, optimizerG, optimizerD, path, args.multi_gpus)
        netG, netD, netC, GALIPNetD, optimizerG, optimizerD = load_models_opt(netG, netD, netC, GALIPNetD, optimizerG, optimizerD,
                                                                              path, args.multi_gpus)
    ...
    for epoch in range(start_epoch, args.max_epoch, 1):
        ...
        torch.cuda.empty_cache()
        train(train_dl, netG, netD, netC, GALIPNetD, text_encoder, image_encoder, optimizerG, optimizerD, scaler_G, scaler_D, args)
        torch.cuda.empty_cache()
        # save
        if epoch%save_interval==0:
            #save_models_opt(netG, netD, netC, optimizerG, optimizerD, epoch, args.multi_gpus, args.model_save_file)
            save_models_opt(netG, netD, netC, GALIPNetD, optimizerG, optimizerD, epoch, args.multi_gpus, args.model_save_file)
            torch.cuda.empty_cache()
        # sample
        ...
        if epoch%test_interval==0:
            #torch.cuda.empty_cache()
            FID, TI_score = test(valid_dl, text_encoder, netG, CLIP4evl, args.device, m1, s1, epoch, args.max_epoch, args.sample_times, args.z_dim, args.batch_size, args.multi_gpus)
            torch.cuda.empty_cache()
        if (args.multi_gpus==True) and (get_rank() != 0):
            None
        else:
