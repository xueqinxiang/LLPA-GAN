...
def choose_GALIP_model(model):
    NetG = GALIP.NetG
    NetD = D.NetD
    NetC = GALIP.NetC
    CLIP_IMG_ENCODER = GALIP.CLIP_IMG_ENCODER
    CLIP_TXT_ENCODER = GALIP.CLIP_TXT_ENCODER

    # *************** 20230925
    GALIPNetD = GALIP.NetD
    # *************** 20230925

    return NetG, NetD, NetC, CLIP_IMG_ENCODER, CLIP_TXT_ENCODER, GALIPNetD

def choose_model(model):
    NetG = G.NetG
    NetD = D.NetD
    NetC = G.NetC
    CLIP_IMG_ENCODER = G.CLIP_IMG_ENCODER
    CLIP_TXT_ENCODER = G.CLIP_TXT_ENCODER

    # *************** 20230925
    GALIPNetD = D.GALIPNetD
    # *************** 20230925
...
def load_models_opt(netG, netD, netC, GALIPNetD, optim_G, optim_D, path, multi_gpus):
    ...
    GALIPNetD = load_model_weights(GALIPNetD, checkpoint['model']['GALIPNetD'], multi_gpus)
    ...
    return netG, netD, netC, GALIPNetD, optim_G, optim_D
...
def save_models_opt(netG, netD, netC, GALIPNetD, optG, optD, epoch, multi_gpus, save_path):
    if (multi_gpus==True) and (get_rank() != 0):
        None
    else:
        state = {'model': {'netG': netG.state_dict(), 'netD': netD.state_dict(), 'netC': netC.state_dict(), 'GALIPNetD': GALIPNetD.state_dict()}, \
