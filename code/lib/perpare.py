...
import clip
import importlib
from lib.utils import choose_model,choose_GALIP_model
from networks.generator import MogLSTM
...
def prepare_models(args):
    ...
    # *************** 20230925
    NetG, NetD, NetC, CLIP_IMG_ENCODER, CLIP_TXT_ENCODER, GALIPNetD = choose_model(args.model)
    ##NetG, NetD, NetC, CLIP_IMG_ENCODER, CLIP_TXT_ENCODER, GALIPNetD = choose_GALIP_model(args.model) # ******GALIP OK
    # *************** 20230925
    ...
    CLIP_txt_enc.eval()
    # lstm models
    #lstm = MogLSTM(868, 868, 5)  #768 + 100 ViT-L/14
    lstm = MogLSTM(612, 612, 5)  # 512 + 100 ViT-B/32
    # GAN models
    #netG = NetG(args.nf, args.z_dim, args.cond_dim, args.imsize, args.ch_size, args.mixed_precision, CLIP4trn).to(device)
    netG = NetG(args.nf, args.z_dim, args.cond_dim, args.imsize, args.ch_size, args.mixed_precision, CLIP4trn, lstm).to(device)
    #netG = NetG(args.nf, args.z_dim, args.cond_dim, args.imsize, args.ch_size, args.mixed_precision, CLIP4trn).to(device) # ******GALIP OK
    #netD = NetD(args.nf, args.imsize, args.ch_size, args.mixed_precision).to(device)
    #netD = NetD(netG.c_dim).to(device)
    netD = NetD(512).to(device) # ******GALIP OK
    netC = NetC(args.nf, args.cond_dim, args.mixed_precision).to(device)

    # *************** 20230925
    GALIPNetD = GALIPNetD(args.nf, args.imsize, args.ch_size, args.mixed_precision).to(device)
    # *************** 20230925
    ...
    if (args.multi_gpus) and (args.train):
        ...
        netC = torch.nn.parallel.DistributedDataParallel(netC, broadcast_buffers=False,
                                                          ...
                                                          output_device=local_rank, find_unused_parameters=True)
        # *************** 20230925
        GALIPNetD = torch.nn.parallel.DistributedDataParallel(GALIPNetD, broadcast_buffers=False,
                                                          device_ids=[local_rank],
                                                          output_device=local_rank, find_unused_parameters=True)
        # *************** 20230925
    return CLIP4trn, CLIP4evl, CLIP_img_enc, CLIP_txt_enc, netG, netD, netC, GALIPNetD
